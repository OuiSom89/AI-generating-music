...
encode = lambda s: [hyperparameters["stoi"][c] for c in s]
decode = lambda l: ''.join([hyperparameters["itos"][str(i)] for i in l])
...
model.load_state_dict(torch.load(f"{os.path.dirname(os.path.abspath(__file__))}\IA_parameters.pth"))
model.eval()

while True:
    context_text = input("Entrez votre contexte initial : ")
    context = torch.tensor(encode(context_text), dtype=torch.long, device=device).unsqueeze(0)
    print(context)
    model.generate(context, max_new_tokens=int(input()))[0].tolist()